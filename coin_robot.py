...
class coin_robot:
    ...
    def solve(self):
        ...
        for i in range(self.row):
            for j in range(self.column):
                better_move = max(F[i - 1][j], F[i][j - 1])
                F[i][j] = better_move + self.map[i][j]
                if better_move == F[i - 1][j]:
                    path[(i, j)] = (i - 1, j)
                elif better_move == F[i][j - 1]:
                    path[(i, j)] = (i, j - 1)
        print(F)

        path_list = []
        child = (self.row - 1, self.column - 1)
        path_list.append(child)
        while True:
            child = path[child]
            if child == (-1, 0):
                break
            path_list.insert(0, child)

        # Modify this line to call draw() to draw the path
        self.draw(F[-1][-1], path_list)
